...
import CardHOC from '@features/checkout/CardHOC';

const CustomTabBar: FC<BottomTabBarProps> = props => {
  const isVegMode = useAppSelector(state => state.user.isVegMode);
  const {scrollY} = useSharedState();
  const {state, navigation} = props;
  const buttom = useSafeAreaInsets();
  ...
  return (
    <>
    {isLiveTabFocused && <CardHOC />}
      <Animated.View
        style={[
          styles.tabBarContainer,
          animatedStyle,
          {
            paddingBottom: buttom.bottom,
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            Alert.alert('Grocery Logo Clicked');
          }}>
          <Image
